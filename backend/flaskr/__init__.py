...
def create_app(test_config=None):
  ...
  def get_questions():
    ...
    current_questions = paginate_questions(request,questions) #give format to the questions and applying pagination
  ...
  def get_questions_quizzes():
    ...
    try: 
      ...
      #Conditional: if there id ==0, meaning that the user chose ALL and the previous questions is empty. I will select all the questions
      if category_id ==0 and previous_questions == []:
        questions = Question.query.all()

      #If not I will get the questions by the category sent in the request
      questions =  Question.query.filter(Question.category == category_id)

      #I iterate each question to check if the question is part of the previos question
      for question in questions:
        if question.id not in previous_questions:
          result_questions.append(question.format()) #if not the question is added to the list result_questions
          success = True
      
      #Return json with empty list or with results
      return jsonify({"success": success,
                      "question": random.choice(result_questions) if len(result_questions)!=0 else []})
